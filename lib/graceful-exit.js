  ...
  return function checkIfExitingGracefully (req, res, next) {
    if (app.settings.graceful_exit === true) {
      if (options.respondDuringExit) {
        logger('Server unavailable, incoming request rejected with error: ' + req && req.headers || '?');
        ...
        logger('Server unavailable, incoming request dropped silently: ' + req && req.headers || '?');
